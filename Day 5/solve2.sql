...
trimmed_ranges AS (
    SELECT DISTINCT a, (SELECT MAX(b) FROM ranges WHERE ranges.a == q.a) AS b FROM ranges AS q
)
-- SUM(MAX(b - a + 1, 0))
SELECT SUM(MAX(b - a + 1, 0)) FROM (SELECT (
    SELECT CASE COUNT(*) WHEN 0 THEN q.a ELSE MAX(b) + 1 END FROM trimmed_ranges WHERE (a < q.a AND q.a <= b) AND NOT(b == q.b AND a == q.a)
) AS a, q.b FROM trimmed_ranges AS q) as disjoints;
